...
public class PaginaUnoService {
	...
	public boolean addFirstPage(Document document, PdfWriter writer, String anual, String fechaFinElaboracion) {
		try {
			...
			try {
				InputStream imageStream = getClass().getClassLoader().getResourceAsStream("static/image1.jpeg");
				Image image1 = Image.getInstance(reportDataService.toByteArray(imageStream));
			...
			try {
				...
				InputStream imageStream3 = getClass().getClassLoader().getResourceAsStream("static/image3.png");
				Image image3 = Image.getInstance(reportDataService.toByteArray(imageStream3));
